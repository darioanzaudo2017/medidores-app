    ...
    const fetchDashboardData = async () => {
        try {
            ...
            const { count: completedOrders } = await supabase
                ...
                .in('id_estado_orden', ['71d96e8b-18b5-41c8-9aff-cfd0c708f526', 'b28d55bb-f885-4cfa-a181-88c1d80ac118']); // VERIFICADO or CERRADO AGENTE
            ...
            const { count: agentsCount } = await supabase
                .from('t_usuarios')
                .select('*', { count: 'exact', head: true })
                .eq('tipo_usuario_id', (await supabase.from('t_tipos_usuario').select('id').eq('nombre', 'AGENTE').single()).data?.id);

            // 2. Fetch Agent Performance (Simplified)
            const { data: agentsData } = await supabase
                ...
                .select(`
                    ...
                    nombre, 
                    apellido,
                    tipo_usuario:t_tipos_usuario(nombre)
                `);

            console.log('Agents fetched for dashboard:', agentsData?.length);

            setStats({
                totalOrders: totalOrders || 0,
                completionRate: totalOrders ? Math.round(((completedOrders || 0) / totalOrders) * 100) : 0,
                avgTime: '4.2h', // Placeholder for complex calc
                activeAgents: agentsCount || 0,
                pendingOrders: pendingOrders || 0
            });

            // Mocking performance for the premium look
            setAgentPerformance([
                { name: 'Marco Verratti', role: 'Técnico de Campo', assigned: 164, completed: 156, score: 4.8 },
                { name: 'Sarah Jenkins', role: 'Agente de Soporte', assigned: 150, completed: 142, score: 5.0 },
                { name: 'David Chen', role: 'Especialista Eléctrico', assigned: 160, completed: 138, score: 3.5 },
            ]);
                            ...
                            {[60, 45, 80, 55, 90, 70, 85].map((val, i) => (
                                <div key={i} className="flex-1 flex flex-col items-center gap-3 group">
                                    <div className="w-full flex flex-col items-center justify-end gap-1 h-full">
                                        <div
                                            className="w-full bg-gray-100 rounded-t-xl transition-all group-hover:bg-gray-200"
                                            style={{ height: `${val + 10}%` }}
                                        ></div>
                                        <div
                                            className="w-full bg-blue-600 rounded-t-xl transition-all group-hover:bg-blue-700 shadow-lg shadow-blue-100"
                                            style={{ height: `${val}%` }}
                                        ></div>
                                    </div>
                    ...
                    <div className="col-span-12 lg:col-span-4 bg-white p-8 rounded-[2.5rem] border border-gray-50 shadow-sm text-center flex flex-col justify-center">
                        ...
                        <div className="relative size-48 mx-auto flex items-center justify-center">
                            <svg className="size-full -rotate-90" viewBox="0 0 36 36">
                                ...
                                <path className="text-blue-600" strokeWidth="3" strokeDasharray="65, 100" strokeLinecap="round" stroke="currentColor" fill="none" d="M18 2.0845 a 15.9155 15.9155 0 0 1 0 31.831 a 15.9155 15.9155 0 0 1 0 -31.831" />
                                <path className="text-orange-400" strokeWidth="3" strokeDasharray="22, 100" strokeDashoffset="-65" strokeLinecap="round" stroke="currentColor" fill="none" d="M18 2.0845 a 15.9155 15.9155 0 0 1 0 31.831 a 15.9155 15.9155 0 0 1 0 -31.831" />
                            </svg>
                            <div className="absolute inset-0 flex flex-col items-center justify-center">
                                <span className="text-3xl font-black text-slate-800">{stats.pendingOrders}</span>
                        ...
                        <div className="grid grid-cols-2 gap-4 mt-10">
                            <div className="flex items-center gap-2">
                                ...
                                <span className="text-[10px] font-bold text-slate-600 uppercase">Cerradas (65%)</span>
                            </div>
                            <div className="flex items-center gap-2">
                                <span className="w-2.5 h-2.5 rounded-full bg-orange-400"></span>
                                <span className="text-[10px] font-bold text-slate-600 uppercase">En Proceso (22%)</span>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-8 gap-4">
                        ...
                        <div className="flex items-center gap-3">
                            <div className="relative">
                                ...
                                <input className="pl-9 pr-4 py-2 rounded-xl border border-gray-100 bg-gray-50 text-xs font-bold w-48 focus:ring-2 focus:ring-blue-100 transition-all" placeholder="Filtrar agentes..." type="text" />
                            </div>
                        </div>
                    </div>
                            ...
                            <tbody className="divide-y divide-gray-50">
                                {agentPerformance.map((agent, i) => (
                                    <tr key={i} className="hover:bg-gray-50/50 transition-colors group">
                                        ...
                                        <td className="py-5 px-2">
                                            <div className="flex items-center gap-3">
                                                <span className="text-xs font-black text-slate-800">{Math.round((agent.completed / agent.assigned) * 100)}%</span>
                                                <div className="w-16 h-1.5 bg-gray-100 rounded-full overflow-hidden">
                                                    <div
                                                        className="h-full bg-green-500 rounded-full"
                                                        style={{ width: `${(agent.completed / agent.assigned) * 100}%` }}
                                                    ></div>
                                                </div>
                                            </div>
                                        </td>
